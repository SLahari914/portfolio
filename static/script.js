...
/* ==========================================
   10. CONTACT FORM — Flask-Mail via fetch
   ========================================== */
const contactForm = document.getElementById('contactForm');
const formSuccess = document.getElementById('formSuccess');
const formError = document.getElementById('formError');
const formErrorTxt = document.getElementById('formErrorText');
...
if (contactForm) {
    contactForm.addEventListener('submit', async (e) => {
        ...
        if (!name || !email || !message) {
            formErrorTxt.textContent = 'Please fill in all required fields.';
        ...
        try {
            ...
            if (response.ok && result.success) {
                ...
                formErrorTxt.textContent = result.message || 'Something went wrong. Please try again.';
                formError.classList.add('show');
                setTimeout(() => formError.classList.remove('show'), 6000);
            }
        } catch (err) {
            formErrorTxt.textContent = 'Network error. Please check your connection and try again.';
            formError.classList.add('show');
            setTimeout(() => formError.classList.remove('show'), 6000);
        } finally {
